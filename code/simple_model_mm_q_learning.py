...
import pandas as pd
from utils.simple_model.plotting import heatmap_Q, show_Q
from environments.simple_model.simple_model_mm import *
...
def tabular_Q_learning(
    ...
    alpha_cutoff : float (0<x≤1)
        the proportion of the total number of episodes at which the alpha_end is reached
...
def _plot_rewards_helper(x, smoothing=0.99):
    """
    calculate the smoothed reward from the normal reward
    ...
    smoothing : float
        how much the rewards will be smoothed (<1)

    Returns
    -------
    smooth_rewards : np.array
        an array of the smoothed rewards
    """
...
def plot_rewards(rewards, x_values):
    """
    plots the smoothed reward from the normal reward
